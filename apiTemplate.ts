...
export default class MyGenerator extends CodeGenerator {
  getInterfaceContent(inter: Interface) {
    ...
    const formParameters = inter.parameters.filter(
      item => item.in == "formData"
    );
    const isOnlyQueryParameter =
      queryParameters.length > 0 &&
      bodyParameters.length == 0 &&
      formParameters.length == 0;
    const isOnlyBodyParameter =
      bodyParameters.length > 0 &&
      queryParameters.length == 0 &&
      formParameters.length == 0;
    const isOnlyFormParameter =
      formParameters.length > 0 &&
      bodyParameters.length == 0 &&
      queryParameters.length == 0;
    const hasQueryOrBodyParameter =
      (bodyParameters.length ||
        queryParameters.length ||
        formParameters.length) > 0;

    // eslint-disable-next-line
    let { funcParamsType, exportParamsType, requestParamsType } =
      this.getParametersDefine(
        inter,
        bodyParameters,
        queryParameters,
        formParameters,
